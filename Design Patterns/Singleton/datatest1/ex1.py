...
class Data:
    ...
    [object
        def__new__(cls):] когда вызыва5еться функция syper, в нашем классе,то мы получаем ссылку на базовый класс, и в этом базовом классе,через свою ссылку вызываеться магический метод new,  и этот магический метод уже запускает процес создания екземпляра класса и возвращает адрес нового созданного обьекта. а мы с помощью return возвращаем этот адрес дальше"""
